...
class TestLoginLogout(object):
    ...
    def test_login_logout(self):
        ...
        with allure.step("用户登录"):
            ...
            assert_that(login_resp.status_code,description=f"status_code not right").is_equal_to(200)
            logger.info(f"login_resp:{login_resp.json()}")
        with allure.step("用户登出"):
            logout_resp = flask_clinet.logout()
            assert_that(logout_resp.status_code,description=f"status_code not right").is_equal_to(200)
